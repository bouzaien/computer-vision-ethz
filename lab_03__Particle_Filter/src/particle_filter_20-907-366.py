...
class Simulator(object):
	...
	def getMeasurementModel(self, locations):
		...
		measurement = 1 - (1 - door_1) * (1 - door_2) * (1 - door_3)
	...
	def moveParticles(self, particles, uk):
		...
		Tips:
			* Each particle has to evolve indenpently, so you are supposed
		...
		noise = np.random.normal(0, self.param.odometry_stdev, len(particles))
		prior_particles += noise + uk # the same dstance is added to all particle, and the noise in particle specific
	...
	def resampleParticles(self, prior_particles, particle_weights):
		...
		# TODO: Change this value for the correct one
		# used resampling whell as described here: https://classroom.udacity.com/courses/cs373/lessons/48704330/concepts/487480820923
		resampled_particles = prior_particles
		
		alpha = np.sum(particle_weights)
		particle_weights = particle_weights / float(alpha) # normalize to get probability-like distribution

		n = len(prior_particles)
		new_particles = list()
		...
		for _ in range(n):
			...
			while beta > particle_weights[index]:
				...
				index = (index + 1) % n
			new_particles.append(prior_particles[index])
